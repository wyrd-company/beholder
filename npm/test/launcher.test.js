...
const launcher = require("../bin/beholder.js");
...
test("launcher fails with actionable output when postinstall produced no executable", () => {
  ...
  const child = launcher.run([], {
    binary: "/missing/beholder-native",
  ...
  assert.match(errors[0], /reinstall @wyrd-company\/beholder/);
});
...
test("launcher forwards arguments and inherited standard streams", () => {
  ...
  launcher.run(["status", "--json"], {
    binary: "/package/bin/beholder-native",
    exists: () => true,
    platform: "linux",
    processObject,
    spawn: (...args) => {
  ...
  assert.deepEqual(invocation, [
    "/package/bin/beholder-native",
    ["status", "--json"],
    { argv0: "beholder", stdio: "inherit" },
  ]);
