...
function temporaryDirectory() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "beholder-installer-test-"));
}
...
test("selectTarget maps only release-matrix platforms", () => {
  assert.equal(installer.selectTarget("linux", "x64").asset, "beholder-linux-x86_64.tar.gz");
  assert.equal(installer.selectTarget("linux", "arm64").asset, "beholder-linux-arm64.tar.gz");
  assert.equal(installer.selectTarget("darwin", "arm64").asset, "beholder-macos-arm64.tar.gz");
  assert.equal(installer.selectTarget("win32", "x64").asset, "beholder-windows-x86_64.zip");
...
test("releaseUrls binds the package version and exact asset", () => {
  ...
  assert.deepEqual(installer.releaseUrls("1.2.3", target), {
    archive:
      "https://github.com/wyrd-company/beholder/releases/download/1.2.3/beholder-linux-x86_64.tar.gz",
    checksums: "https://github.com/wyrd-company/beholder/releases/download/1.2.3/SHA256SUMS",
  });
...
test("checksum evidence must be unique and must match the archive", () => {
  const directory = temporaryDirectory();
  const archive = path.join(directory, "beholder-linux-x86_64.tar.gz");
  fs.writeFileSync(archive, "verified bytes");
  const digest = crypto.createHash("sha256").update("verified bytes").digest("hex");
  const expected = installer.parseChecksum(`${digest}  beholder-linux-x86_64.tar.gz\n`, path.basename(archive));
...
test("archive boundaries reject traversal, absolute, foreign, and duplicate executable paths", () => {
  const target = installer.selectTarget("linux", "x64");
  const valid = [
    "beholder-linux-x86_64/",
    "beholder-linux-x86_64/beholder",
    "beholder-linux-x86_64/LICENSE",
  ];
  assert.equal(
...
test("Windows archives use the same directory-prefixed release contract", () => {
  ...
  assert.equal(
    installer.validateArchiveEntries(
      [
        "beholder-windows-x86_64/",
        "beholder-windows-x86_64/beholder.exe",
        "beholder-windows-x86_64/LICENSE",
      ],
      target,
    ),
    "beholder-windows-x86_64/beholder.exe",
  ...
  assert.throws(
    () => installer.validateArchiveEntries(["beholder.exe", "LICENSE"], target),
    /unsafe path/,
  );
});
  ...
  const entries = installer.listArchive("fixture.zip", target, {
    ...
    spawnSync: (executable) => {
      ...
      return {
        status: 0,
        stderr: "",
        stdout: "beholder-windows-x86_64/beholder.exe\n",
      };
    },
  });
  assert.equal(command, "C:\\System\\System32\\tar.exe");
  assert.deepEqual(entries, ["beholder-windows-x86_64/beholder.exe"]);
});
        ...
        spawnSync: (_command, args) => {
          if (args[0] === "-tzf") {
            return {
              status: 0,
              stderr: "",
              stdout: "beholder-linux-x86_64/beholder\n",
            };
          }
...
test("extractExecutable places only the expected executable with executable permissions", () => {
  const directory = temporaryDirectory();
  const fixture = path.join(directory, "fixture", "beholder-linux-x86_64");
  fs.mkdirSync(fixture, { recursive: true });
  fs.writeFileSync(path.join(fixture, "beholder"), "native executable");
  fs.writeFileSync(path.join(fixture, "README.md"), "documentation");
  const archive = path.join(directory, "beholder-linux-x86_64.tar.gz");
  const packed = spawnSync(
    "tar",
    ["-czf", archive, "-C", path.join(directory, "fixture"), "beholder-linux-x86_64"],
    { encoding: "utf8" },
  );
  assert.equal(packed.status, 0, packed.stderr);

  const destination = path.join(directory, "output", "beholder-native");
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(destination)), ["beholder-native"]);
  fs.rmSync(directory, { recursive: true, force: true });
});
