...
function executablePath(platform = process.platform, directory = __dirname) {
  const name = platform === "win32" ? "beholder-native.exe" : "beholder-native";
...
function run(argv = process.argv.slice(2), options = {}) {
  ...
  if (!exists(binary)) {
    (options.error || console.error)(
      "beholder: native executable is missing; reinstall @wyrd-company/beholder or install beholder-cli with Cargo",
    );
    processObject.exitCode = 1;
    return null;
  }

  const child = spawnProcess(binary, argv, { argv0: "beholder", stdio: "inherit" });
  ...
  child.once("error", (error) => {
    cleanup();
    (options.error || console.error)(`beholder: ${error.message}`);
    processObject.exitCode = 1;
  });
