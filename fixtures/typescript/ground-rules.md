...
# TypeScript fixture corpus ground rules

## Purpose

The TypeScript fixture corpus is controlled, valid TypeScript source used to
evaluate code-graph approaches. Source creates the language entities and
relationships that an evaluation rubric later describes. Fixture creation does
not define graph output.

The canonical language-feature checklist is
[`reports/typescript/synthesis/checklist.md`](../../reports/typescript/synthesis/checklist.md).
It is the research authority for fixture scope.

## Language scope

Every project type-checks and emits with one selected release of the official
Microsoft `typescript` compiler (`tsc`) on the pinned Node.js host. The plan
records that compiler version, the Node.js runtime version, the package manager,
the bundled `lib.*.d.ts` standard-library set, and the effective compiler
options that form program identity. The default compilation context is the
pinned compiler on 64-bit x86 Linux under a current module, resolution, and
target configuration. A project adds another compiler version, host runtime,
module or resolution mode, target or library set, or effective-option set only
when valid checklist coverage requires it. The corpus does not provision
historical toolchains or hosts that are not locally installed.

A compilation context in this corpus is the tuple of pinned compiler version,
host runtime, module and resolution mode, target and library set, and effective
compiler options. It is the TypeScript equivalent of a build context.
...
- Conditions outside declared compilation contexts.
...
fixture content. Where a checklist item names failure or diagnostic variants,
only its valid-source obligation is corpus content.

## Project model

The corpus contains as many projects as its language coverage naturally
requires. Each project represents an idiomatic TypeScript library, package,
workspace, or other coherent source tree. Projects need not share a theme,
size, package layout, or source shape.
...
- Occupies one exclusive directory under `fixtures/typescript/projects/`.
...
Projects use the pinned toolchain, project-local packages, or vendored
third-party packages. The pinned `typescript` compiler and Node.js host are the
toolchain rather than per-project dependencies; a project vendors them and any
other dependency so that validation runs without network access. A reviewed
project brief names every third-party dependency beyond the toolchain. An
implementor may access the network only to acquire those named dependencies and
the pinned toolchain.

Vendored third-party source includes:

- A pinned package version or source revision.
- Source location.
- Required license and notice files.
- Package metadata needed for offline installs and resolution.
...
TypeScript has no test-only language construct: every language feature is
expressible in ordinary source. A project therefore plans tests only in the
rare case that test source is itself the coverage, and otherwise records
`none`.
...
- `build` type-checks and emits all fixture-owned sources in every declared
  compilation context, using `tsc` or `tsc -b` for solution builds.
- `lint` checks formatting with the pinned formatter and runs the compiler as a
  no-emit checking pass over fixture-owned sources. Vendored source is
  excluded.
- `test` runs the project's tests so any included test source compiles and
  executes on the pinned host. It succeeds when a project has no test files.

All three tasks pass. They are thin wrappers around ordinary toolchain
commands. They do not write validation logs, evidence records, manifests,
hashes, or audit output.

A project defines a `generate` task only when generator invocation is fixture
content. `build`, `lint`, and `test` never invoke it.

The corpus root defines the same `build`, `lint`, and `test` tasks and invokes
the corresponding task in every accepted project.

## Coverage files

Each project's `coverage.md` is the source of truth for its primary
assignments. Its Markdown table records for each assigned checklist item:

- Canonical checklist identifier.
- Stable source path.
- Named declaration, directive, configuration, package, or test that creates
  coverage.
- Compilation context when it differs from the default.
...
file, the Go planner prompts as process templates, and the canonical TypeScript
checklist. It may inspect the installed TypeScript toolchain with local shell
and read tools, including registry metadata needed to select an installable
pinned version. It does not browse, perform external research, read research
inputs, inspect unrelated repositories, or inspect prior fixture work.

The plan contains one overview and one standalone brief per project. The
overview:

- Records the selected TypeScript compiler, Node.js host, package manager, and
...
project constraints without designing source. Beyond required `Taskfile.yml`
and `coverage.md`, it does not name fixture source files, fixture-owned modules
or declarations, domain examples, cases, subcases, call sequences, code,
...
read, and write tools inside assigned worktrees. External research and
unrelated workspace reads are not inputs. The only network exception is
acquisition of the pinned toolchain and a third-party dependency named by the
reviewed brief.
...
- A dependency is missing, unpinned, unvendored, or missing required
  provenance, license, or notices.
...
- Compiler versions, host runtimes, module modes, targets, or compilation
  contexts outside the project brief.
...
replacement design or judging choices reserved for implementor. Domain
behavior, validators, evidence systems, graph output, rubric assertions,
project count, equal work sizes, and preferred themes remain outside review
scope.
...
Temporary local and remote branches and worktrees are removed after
integration. When Beholder adoption is authorized, selected artifacts enter
`main` through normal commits rather than a merge of unrelated histories.
