...
# C# fixture corpus ground rules

## Purpose

The C# fixture corpus is controlled, valid C# source used to evaluate code-graph
...
[`reports/csharp/synthesis/checklist.md`](../../reports/csharp/synthesis/checklist.md).
It is the research authority for fixture scope.

## Language scope

Every project compiles with one selected release of the official .NET SDK in
SDK-style MSBuild with NuGet `PackageReference`. The plan records that SDK, its
bundled Roslyn and MSBuild, and the Common Language Runtime (CLR) it targets. The
default build context is the SDK's current target framework moniker on 64-bit x86
Linux and the CoreCLR shared framework installed with that SDK. A project selects
an earlier language version through `LangVersion` when a version boundary changes
observable behavior. A project adds another target framework, shared framework,
runtime identifier, unsafe context, publish mode, or non-C# producer only when
valid checklist coverage requires it and that context is locally available.

The corpus does not provision historical .NET SDKs or Roslyn compilers, Windows-
or macOS-only frameworks, non-x64 processes, or external services.
...
diagnostic assertions, parser recovery, and intentionally invalid examples are
not fixture content. An item whose distinctive coverage exists only as a compiler
error, an unresolved-symbol state, or a recovered malformed tree is an invalid-
only exception. Ordinary error counterexamples named beside a valid demonstration
are simply outside fixture source and need no separate exception entry.

## Project model

The corpus contains as many projects as its language coverage naturally requires.
Each project represents an idiomatic C# library, application, multi-assembly
solution, or other coherent source tree. Projects need not share a theme, size,
solution layout, or source shape.
...
- Occupies one exclusive directory under `fixtures/csharp/projects/`.
- Is independently implementable and reviewable.
- Contains every local project and assembly it needs.
...
Projects use the Base Class Library, installed shared frameworks, project-local
projects, or vendored third-party packages. A reviewed project brief names every
third-party dependency. An implementor may access the network only to acquire
those named dependencies.
...
source and any on-disk generated output. Required validation tasks never write or
rewrite tracked source. A Roslyn source generator that runs in-process during
compilation is part of the build and does not write tracked source; it is not a
regeneration step. The plan may require a separate `generate` task only when a
generator invocation writes committed source that later validation must not
change.
...
- `build` compiles all fixture-owned projects in every declared build context
  without restoring from the network.
- `lint` verifies formatting of fixture-owned source and honors the Roslyn
  analyzers the build already runs. Vendored source is excluded.
- `test` runs project tests so included test source compiles and executes. It
  succeeds when a project has no test source.

All three tasks pass. They are thin wrappers around ordinary `dotnet` commands.
They do not write validation logs, evidence records, manifests, hashes, or audit
output.
...
- Stable source path.
- Named declaration, attribute, target, project, package, or test that creates
  coverage.
...
file and the canonical C# checklist. It may inspect the installed .NET toolchain
...
- Records the selected .NET SDK, bundled Roslyn and MSBuild, target CLR, and
  available build contexts.
...
`coverage.md`, it does not name fixture source files, fixture-owned projects,
assemblies, namespaces, types, members, or other declarations, domain examples,
cases, subcases, call sequences, code, pseudocode, validation internals, or
implementation steps. The implementor owns those choices. A third-party package
named under dependency needs is not a fixture-owned source-design choice.
...
- .NET SDK versions, operating systems, architectures, or build contexts outside
  the project brief.
