...
Phase 1 is embarrassingly parallel and deterministic: identical file content always yields an identical result. Nothing is written to disk, and the phase runs in memory on every invocation. That determinism is what would let a cache key on content hash if a codebase ever grows large enough to need one.
...
1. **Symbol-level index.** Every file, function, and type with path, kind, name, and line range. Symbol identity is stable across line shifts, so a reformat is not a change. Everything else is a query over this.
...
7. **Machine-first output.** JSONL primary, SARIF second, human format last. Computed on demand.
...
- **No stored index.** Analysis is fast enough to compute on demand. Phase 1 stays a pure function of file content so a cache is a drop-in addition when a codebase is large enough to need one.
